-- Fix critical finding: any manager account can view every crew member's assessment data

DROP POLICY IF EXISTS "crew_read_own_assessment" ON public.smc_assessments;
    ...
    WHERE mp.id = auth.uid()
      AND (
        EXISTS (
          ...
          WHERE cr.crew_profile_id = public.smc_assessments.crew_profile_id
            AND cr.manager_profile_id = mp.id
            AND cr.status = 'accepted'
        )
        OR EXISTS (
          SELECT 1 FROM public.job_vacancies jv
          JOIN public.contact_requests cr2 ON cr2.vacancy_id = jv.id
          WHERE cr2.crew_profile_id = public.smc_assessments.crew_profile_id
...
GRANT SELECT ON public.smc_assessments TO authenticated;
