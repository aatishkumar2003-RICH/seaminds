CREATE OR REPLACE FUNCTION public.manager_update_application(p_application_id uuid, p_action text, p_joining_date date DEFAULT NULL::date, p_contract_months integer DEFAULT 9)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSIF p_action = 'offer' THEN
    UPDATE job_applications SET outcome = 'offered', offered_at = now(),
      offered_joining_date = p_joining_date,
      placement_end = coalesce(p_joining_date, current_date) + (greatest(coalesce(p_contract_months,9),1) * interval '1 month')::interval
      WHERE id = app.id;
    INSERT INTO notifications (crew_id, kind, title, body, icon, screen) VALUES
      (app.crew_id, 'job_offer', '🎉 JOB OFFER — ' || coalesce(app.rank_applied,'position'),
       coalesce(app.company_name,'A company') || ' wants you on board' ||
       coalesce(', joining ' || to_char(p_joining_date,'DD Mon YYYY'), '') ||
