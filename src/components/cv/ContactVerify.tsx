...
const ContactVerify = ({ channel, value, verifiedValue, onVerified }: ContactVerifyProps) => {
  ...
  const call = async (action: "send" | "verify") => {
    const { data, error } = await supabase.functions.invoke("verify-contact", {
      body: { action, channel, target, code, fallback_email: user?.email || "" },
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    if (error && !data) throw new Error("Could not reach the verification service");
  ...
  const handleSend = async () => {
    setErr(""); setMsg("");
    if (!target) { setErr(`Enter your ${label} first`); return; }
    setSending(true);
    try {
    ...
    <div className="mt-1.5">
      {!codeSent ? (
        <div className="flex items-center gap-2">
          ...
          >
            {sending ? <Loader2 size={11} className="animate-spin" /> : "Send code"}
          </button>
        </div>
      ) : (
        <div className="flex items-center gap-2">
          ...
          >
            {checking ? <Loader2 size={11} className="animate-spin" /> : "Verify"}
          </button>
          <button type="button" onClick={handleSend} disabled={sending} className="text-[11px] text-gray-400 underline">
            Resend
          </button>
        </div>
      )}
